...
# mean of all previous exercise gyro samples
...
# variances
gyroVar = 0.0009311885380426304 # variance of all gyro_xyz variables from previous exercise
pitchVar = 0.14945441086477543
...
for line in f:
	...
	# integrate gyro velocities to relative angles
	...
	predPitch = estPitch + gyro_x * (ts_now - ts_prev)
	gyroVarAcc += gyroVar
	...
	corrPitch = predPitch + K * (pitch - predPitch)
	corrVar = predVar * (1 - K)
	estPitch = corrPitch
	estVar = corrVar
	gyroVarAcc = 0
...
if showPlot == True:
	...
	plt.plot(plotDataGyro, label="Gyro Integrated Angle")
	plt.grid()
	#plt.savefig('imu_exercise_gyro.png')

	#plt.figure(2)
	plt.title('Measured and Kalman Filtered angles')
	plt.plot(plotDataAcc, 'blue', label="Accelerometer Measured Angle")
